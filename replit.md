...
- **Smart Contract Escrow System**: Fully integrated on-chain USDC payment system on the Base blockchain using `USDCEscrow.sol` (457 lines, OpenZeppelin v5) for milestone-based releases, dispute resolution, platform fees, and automated refunds. Frontend includes comprehensive payment flow with USDC approval UI, balance checks, transaction tracking, and BaseScan integration. Backend API endpoints handle escrow order updates and builder notifications.
...
- **Notification System**: Comprehensive notification system with browser push notifications for messages, orders, and milestones. Email notifications are sent when builders receive new messages while offline, ensuring they never miss client inquiries. Email digest system with daily/weekly summaries based on user preferences. Real-time notification counters and read/unread tracking. Builders provide email during onboarding for notification delivery.
...
- **Core Profile Fields**: Name, email (for notifications), headline, bio, detailed description, profile image, portfolio media, video showreel, verified status, category, PSX tier, rating, review count, completed projects, response time, success rate, on-time delivery rate.
